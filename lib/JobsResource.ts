...
export default class JobsResource {
    ...
    async get(id: string, query = null): Promise<Job> {
        const response = await this.OnlineUniConverter.axios.get(`jobs/${id}`, {
            params: query || {}
        });
    ...
    async create(data: JobTemplate | null = null): Promise<Job> {
        const response = await this.OnlineUniConverter.axios.post('jobs', data);
        return response.data.data;
    }

    // async delete(id: string): Promise<void> {
    //     await this.OnlineUniConverter.axios.delete(`jobs/${id}`);
    // }
